...
async fn feed_snapshots(
    ...
    let rpc_client_data = http::connect_with_options::<solana_rpc::rpc::rpc_accounts::AccountsDataClient>(&config.rpc_http_url, true)
    .await.map_err_anyhow()?;

    let rpc_client_scan = http::connect_with_options::<solana_rpc::rpc::rpc_accounts_scan::AccountsScanClient>(&config.rpc_http_url, true)
    .await.map_err_anyhow()?;
