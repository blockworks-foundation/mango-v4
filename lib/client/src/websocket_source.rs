...
use mango_feeds_connector::{
    EntityFilter, FeedFilterType, FeedWrite, FilterConfig, Memcmp,
...
async fn feed_data(
    ...
    let mut jobs: futures::stream::FuturesUnordered<_> = all_jobs        .into_iter()
        .collect();
...
async fn handle_message(name: &str, message: Result<FeedWrite, RecvError>, sender: &Sender<Message>) -> bool {
...
async fn handle_feed_write(sender: &Sender<Message>, data: FeedWrite) {
    match data {
        FeedWrite::Account(account) => {
            sender.send(Message::Account(AccountUpdate::from_feed(account))).await.expect("sending must succeed")
        },
        FeedWrite::Snapshot(mut snapshot) => {
            sender.send(Message::Snapshot(snapshot.accounts.drain(0..).map(|a| AccountUpdate::from_feed(a)).collect(), crate::account_update_stream::SnapshotType::Partial)).await.expect("sending must succeed")
        }
    }
}
