...
describe.only('Oracle', () => {    
    ...
    const secretKey = JSON.parse(fs.readFileSync(process.env.KEYPAIR_PATH as string, 'utf-8'))
  ...
  it.skip('can generate fixed fallback oracles', async () => {
    const client = MangoClient.connect(provider, CLUSTER, MANGO_V4_ID[CLUSTER], {fallbackOracleConfig: [new PublicKey('H6ARHf6YXhGYeQfUzQNGk6rDNnLBQKrenN712K4AQJEG')]}); // SOL
