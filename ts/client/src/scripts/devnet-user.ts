...
import fs from 'fs';
import { PerpOrderType, Side } from '../accounts/perp';
...
async function main() {
  ...
  if (true) {
    ...
    try {
      ...
      console.log(`...placing perp bid ${clientId}`);
      const price =
    ...
    try {
      const clientId = Math.floor(Math.random() * 99999);
      console.log(`...placing perp ask ${clientId}`);
      const price =
        group.banksMapByName.get('BTC')![0].uiPrice! +
        Math.floor(Math.random() * 100);
      const sig = await client.perpPlaceOrder(
    ...
    // console.log(Array.from(bids.items(client, perpMarket!)));
    // const asks: BookSide = await perpMarket?.loadAsks(client)!;
    // console.log(asks.toString());
    // console.log(Array.from(asks.items(client, perpMarket!)));
    ...
    // await mangoAccount.reload(client, group);
    // console.log(mangoAccount.perpOrdersActive());
  }
