...
pub fn token_edit(
    ...
    flash_loan_swap_fee_fraction_opt: Option<f32>,
) -> Result<()> {
    ...
    for ai in ctx.remaining_accounts.iter() {
        ...
        if let Some(fees_fraction) = flash_loan_swap_fee_fraction_opt {
            msg!(
                "Flash loan swap fee fraction old {:?}, new {:?}",
                bank.flash_loan_swap_fee_fraction,
                fees_fraction
            );
            require_gte!(fees_fraction, 0.0); // values <0 are not currently supported
            bank.flash_loan_swap_fee_fraction = fees_fraction;
            require_group_admin = true;
        }
    }
