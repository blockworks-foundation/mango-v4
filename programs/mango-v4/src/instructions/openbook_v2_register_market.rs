...
pub fn openbook_v2_register_market(
    ...
    require!(
        market_external.quote_mint == quote_bank.mint,
        MangoError::SomeError
    );
    require!(
        market_external.base_mint == base_bank.mint,
        MangoError::SomeError
    );

    let mut openbook_v2_market = ctx.accounts.openbook_v2_market.load_init()?;
    
    *openbook_v2_market = OpenbookV2Market {
        ...
        market_index,
        bump: *ctx.bumps.get("openbook_v2_market").ok_or(MangoError::SomeError)?,
        padding2: Default::default(),
        registration_time: Clock::get()?.unix_timestamp.try_into().unwrap(),
        reserved: [0; 512],
    };

    let mut openbook_v2_index_reservation = ctx.accounts.index_reservation.load_init()?;
    *openbook_v2_index_reservation = OpenbookV2MarketIndexReservation {
        group: ctx.accounts.group.key(),
        market_index,
        reserved: [0; 38],
    };
    ...
    emit!(OpenbookV2RegisterMarketLog {
        ...
        openbook_v2_market: ctx.accounts.openbook_v2_market.key(),
        market_index,
        base_token_index: base_bank.token_index,
        quote_token_index: quote_bank.token_index,
        openbook_v2_program: ctx.accounts.openbook_v2_program.key(),
        openbook_v2_program_external: ctx.accounts.openbook_v2_market_external.key(),
    });
