use crate::state::perp::bookside::{BookSide, BookSideType};
use crate::state::perp::nodes::{InnerNode, LeafNode, NodeHandle, NodeRef};
...
impl<'a> BookSideIter<'a> {
    pub fn new(book_side: &'a BookSide, now_ts: u64) -> Self {
        let (left, right) = if book_side.side_type == BookSideType::Bids {
            (1, 0)
        } else {
            (0, 1)
        };
